...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python merge_script.py <dossier> <fichier_principal.py>")
        sys.exit(1)
    ...
    if main_file not in local_files:
        print(f"Erreur: {main_file} non trouvé dans {target_dir}")
        sys.exit(1)
    ...
    # Détection des fonctions dans chaque module
    ...
    for file in order:
        ...
        for line in content:
            ...
            # Remplacement des appels module.fonction()
            for module in local_functions:
                for func in local_functions[module]:
                    pattern = re.compile(rf'\b{module}\.{func}(?=\()')
                    code_part = pattern.sub(func, code_part)
            ...
            if code_part.startswith(('import ', 'from ')):
                ...
                if all(mod not in local_modules for mod in modules):
                    external_imports.add(line.strip())
            else:
                merged_code.append(new_line)

    # Écriture finale
    with open('../build/final.py', 'w') as f:
